...
public class DictionaryManager {
    //maybe a hashtable of hashtables 
    //is going a bit far?
    ...
    public DictionaryManager(){
        ...
        //dictionaries.put("test", 
              //readDict("/home/felix/word-play
        ///dictionaries/test-6-word.txt"));
    }
    ...
    public Hashtable readDict(String fileName){
        ...
        try {
            ...
            ArrayList<String> wordList = new 
                    ArrayList<String>();
            while ((strLine = dictFile.readLine())
                    != null)   {
                wordList.add(strLine.trim()
                        .toLowerCase());
            }
            System.out.println(wordList);
            //figure out how many letters is 
            //in the words here
            int wordLength = wordList.get(0).length()-1;
            //throw errors if there are different
            //amounts of letters
            //put words into a hashtable where 
            //one word differences are the values.
            String curWord;
            ArrayList<String> curMatchList = 
                    new ArrayList<String>();
            for (int i=0;i<wordList.size();i++){
                curWord = wordList.get(i);
                checkWordLength(curWord, wordLength);
                curMatchList = findMatches(
                        curWord, wordList);
    ...
    private static void checkWordLength(
            String word, int thisLength) 
    ...
    /**
     * finds all words that are one letter 
     * different to a given word from
     * a provided list of words. it returns a 
     * list of matches.
     * @param word
     * @param wordList
     * @return 
